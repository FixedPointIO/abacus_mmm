"""A simple and abacus library for Media Mix Modelling.
...
def _compare_equality_for_lmmm(item_1: Any, item_2: Any) -> bool:
  ...
  Helper function for the __eq__ method of abacusmMM. First checks if items
  ...
  abacusMMM. Sometimes the DeviceArray is hidden in the value of a
...
class abacusMMM:
  """abacus Media Mix Modelling wrapper for bayesian models.
  ...
  def __eq__(self, other: Any) -> bool:
    """Equality method for abacusMMMM.

    We need a special method here to handle a couple of issues. First, some of
    the attributes for abacusMMM are arrays, which contain multiple values
    ...
    Returns:
      ...
      abacusMMM.
    """
    if not isinstance(other, abacusMMM):
  ...
  def get_posterior_metrics(
    ...
    if not hasattr(self, "trace"):
      raise NotFittedModelError(
          "abacusMMM has not been fitted and cannot run estimations. "
