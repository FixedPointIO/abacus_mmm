...
def _make_single_prediction(media_mix_model: abacus_mmm.abacusMMM,
  ...
  abacus_mmm.abacusMMM for regular predict functionality.
...
def _generate_diagonal_predictions(
    media_mix_model: abacus_mmm.abacusMMM,
...
def _calculate_media_contribution(
    media_mix_model: abacus_mmm.abacusMMM) -> jnp.ndarray:
  ...
  if not hasattr(media_mix_model, "trace"):
    raise abacus_mmm.NotFittedModelError(
...
def plot_model_fit(media_mix_model: abacus_mmm.abacusMMM,
                   target_scaler: Optional[preprocessing.CustomScaler] = None,
                   interval_mid_range: float = .9,
...
def plot_out_of_sample_model_fit(out_of_sample_predictions: jnp.ndarray,
                                 ...
                                 media_mix_model: abacus_mmm.abacusMMM = None,
                                 interval_mid_range: float = .9,
                                 digits: int = 3) -> matplotlib.figure.Figure:
...
def plot_bars_media_metrics(
  ...
  Args:
    metric: Estimated media metric as returned by
      abacus_mmm.get_posterior_metrics(). Can be either contribution
      percentage or ROI.
